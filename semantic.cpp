...
void staticSem(Node* node, int sCount, int varCount) {
    ...
    switch(node->code) {
        case PROGRAM:   //<program> -> <vars> main <block>
            //cout << "Found PROGRAM\n";
            sCount = 0;
            staticSem(node->child1, sCount, varCount);
            staticSem(node->child2, sCount, varCount);
        ...
        case IF:        // <if> -> if [ <expr> <RO> <expr> ] then <stat>
            ...
            staticSem(node->child1, sCount, varCount);
            staticSem(node->child2, sCount, varCount);
            staticSem(node->child3, sCount, varCount);
            staticSem(node->child4, sCount, varCount);
            return;
